...
class Bestellung_aufgeben:
    ...
    def bestellung_auswerten(self):
        ...
        self.ausgabe_bestellung(aktuelle_bestellung_id,self.kunde_id)

    def ausgabe_bestellung(self, aktuelle_bestellung_id, kunde_id):
        ...
        query_kundendaten = ("SELECT vorname, nachname, adresse_id FROM kunde WHERE kunde_id="+kunde_id+" ")
...
""" print("hallo1")
        ...
        db_cursor.execute(query_kundendaten)
        for(vorname, nachname, adresse_id) in db_cursor:
            print ('</table>\
            <p>'+self.vorname+' '+self.nachname+' '+str(self.adresse_id)+'</p></body>')
        print(vorname)
        print("hallo")
        kundendaten=db_cursor.fetchall()
        print(kundendaten) #ist leer []
